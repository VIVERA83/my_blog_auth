...
class EmailMessageService(BaseAccessor):
    ...
    async def connect(self):
        ...
        self.logger.info("Connecting to http://localhost:8003/ems")

    async def disconnect(self):
        await self._session.close()
        self.logger.info("Disconnect from to http://localhost:8003/ems")
    ...
    async def send_message_to_confirm_email(
        ...
        async with self._session.post("http://0.0.0.0:8003/ems/send_email",
            ...
            if response.status != 200:
                ...
                assert False, "It was not possible to send an email message"
