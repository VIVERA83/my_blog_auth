...
class UserManager(BaseAccessor):
    ...
    async def user_registration(
        ...
        try:
            async with self.app.postgres.session.begin().session as session:
                ...
                # TODO: Нужно переделать, так как блог перенесли
                # query_user_blog = self.app.postgres.get_query_create_user(
    ...
    async def reset_password(self, email: EmailStr):
        ...
        token = self.app.store.token.create_reset_token(user.id, user.email)
        print(token)
        flag = False
        try:
